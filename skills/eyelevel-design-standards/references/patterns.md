...
## 10. Two-part eyebrow with middle dot

A single uppercase eyebrow split into two clauses joined by a U+00B7 middle dot — `DIFFERENTIATION · CAPABILITIES`, `SCOPE · FULL PIPELINE`, `OBSERVABILITY · INSPECTABLE`. The first clause names the topic; the second clause names the angle.

Rules:

- The middle dot is **typed literally** in source content; do not insert it via CSS `content` or `::before`. This survives translation and plain-text export.
- Both halves share the same weight, size, and tracking as a regular eyebrow — the dot is the only visual differentiator.
- Use only on surfaces where multiple two-part eyebrows appear in parallel (Q&A panels, RFP-response slides, white-paper section openings). Don't mix one two-part eyebrow with single-clause eyebrows on the same surface.

See `voice.md` § 3b for the rhetorical role.

## 11. "Built for X" parallel commitments

A closing scaffold: three cards, each with a `BUILT FOR <X>` eyebrow over a one-line headline and a 2-sentence body. The repetition of "Built for…" turns three feature claims into three parallel commitments — the audience reads it as "we've thought about who you are", not as a feature list.

Rules:

- **Three cards, not four.** Same constraint as pattern 3 — four reads as a bento grid.
- **2 + 1 alternation.** Render two cards on one surface treatment (white) and the third on the other (navy or green) so the row reads as two-and-the-one. The middle card is the navy/green one and carries the strongest claim.
- **Eyebrow scaffold:** all three eyebrows share the `BUILT FOR <X>` shape — `BUILT FOR YOUR ENVIRONMENT`, `BUILT FOR THE PROBLEM`, `BUILT FOR THE SCALE`. Don't break the parallelism.

See `voice.md` § 3a for the rhetorical role.

## 12. Findings cell with em-dash divider

A small card-shaped block — usually appearing in a 2×2 grid — where an eyebrow-style headline (sentence-case, not all-caps) sits above a body sentence, separated by a standalone em-dash on its own line.

Rules:

- The em-dash (`—`) is **typed literally**, never inserted via CSS `content`, never an `<hr>`. It's a quiet vertical pause inside the cell, not a horizontal rule.
- Use on findings / commitments / audit-report grids where each cell is a small concept that reads naturally as a "label · pause · explanation" rhythm. Pairs with meta-headers above the grid (`WHAT WE FOUND` / `OUR COMMITMENT`) that mark the rhetorical role.
- Don't apply this to every grid — it's a layout-specific convention, not a default option.

See `voice.md` § 4b and `eyelevel-slides/references/layouts.md` § 12.
