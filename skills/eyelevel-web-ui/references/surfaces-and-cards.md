...
The EyeLevel visual language is **flat**. Cards sit on a section canvas (white, gray, tint, cyan, green, or navy — see the "Colored / accent surfaces" table below) with a hairline border; there are no drop shadows, no Material elevation, and no layered z-index depth. This is intentional — the green (primary) + coral (accent) colors do the visual work, so surfaces stay calm.
...
Older codebases commonly use both, inconsistently. The canonical answer in any EyeLevel project is `GxCard` (which is just a `Box`) because:
...
If you find code using `<Card>`, replace it the next time you touch the file — it's a safe change.

## Radii by surface type

The ladder itself (4 / 6 / 12 / 20 / 200 px) is defined in `../../eyelevel-design-standards/references/tokens.md` § 5 — don't restate it; import the tokens by name. The table below just maps each surface type onto the right token.
...
A row of two paired surfaces (info card + content card, list + detail, image + copy, info card + upload zone) often uses asymmetric corners on desktop: rounded-outer corners only, flat where the two meet, so the pair reads as one unit. On mobile the two stack vertically and each gets full rounded corners. Build this via responsive `borderRadius` in `sx`:
...
See `examples/ExampleDashboardPage.tsx > BucketRow` for one full composition (info card + upload zone in the GroundX dashboard); the pattern applies to any analogous paired row.
...
Hex values for every color below live in `../../eyelevel-design-standards/references/tokens.md` § 1.1.
...
- Eyebrows swap by surface luminance: Coral eyebrows on White / Gray / Tint / Cyan / Green; Green eyebrows on Navy. See `../../eyelevel-design-standards/references/colors.md` rule 5.
