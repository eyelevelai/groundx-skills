...
Web-UI also defines a handful of chrome-only neutrals and state colors (`NAV_ICON_GREY`, `DISABLED_GREY`, `ROW_SELECTED_BG`, `WARNING_AMBER`) that never appear on slides; those stay scoped to the web-ui skill. See `groundx-web-ui/references/tokens.md` for the full set.
...
### 1.4 Legacy color aliases (retained to keep old builds green)

| Old name (CSS) | Resolves to | Notes |
| --- | --- | --- |
| `--gx-aqua` | `--gx-tint` | Old page / slide backgrounds named "aqua". |
| `--gx-pale-aqua` | `--gx-cyan` | Old accent name. |
| `--gx-active-green` | `--gx-green` | Old emphasis-green name. |
| `--gx-light-grey` | `--gx-gray` | Old gray-surface alias. |

Don't introduce new call sites using the legacy names; they exist only so existing HTML doesn't break. The web-ui skill maintains its own set of TypeScript aliases (`CORAL_ORANGE`, `MAIN_BLACK`, etc.) — see `groundx-web-ui/references/tokens.md` § Legacy aliases.

### 1.5 Hex values are sacred
...
Deprecated: `BORDER_RADIUS_3X` (18), `BORDER_RADIUS_4X` (24), `CARD_BORDER_RADIUS` (12 as a spacing unit). See web-ui tokens for the aliases that keep old code compiling.
...
### 9.2 `groundx-web-ui` (React + MUI + TypeScript)

The web-ui consumes brand tokens via three files under `templates/`:
...
- `constants.chrome.ts` — hand-written dashboard-shell tokens that are *not* brand palette (`drawerWidth`, `NAV_ICON_GREY`, `DISABLED_GREY`, `ROW_SELECTED_BG`, `WARNING_AMBER`, premium-button gradient).
- `constants.legacy.ts` — hand-written deprecated aliases (`CORAL_ORANGE`, `MAIN_BLACK`, `BORDER_RADIUS_3X`, …) that re-export canonical tokens under their old names so pre-2026 imports keep compiling.

`templates/constants.ts` is a thin barrel: `export * from "./constants.generated"; export * from "./constants.chrome"; export * from "./constants.legacy";` so `import { NAVY } from "./constants"` keeps working everywhere.
...
3. Run the codegen: `cd skills/groundx-design-standards && node scripts/generate-mirrors.mjs`. This overwrites `groundx-web-ui/templates/constants.generated.ts` and re-splices the `:root` block of `groundx-slides/templates/styles.css`.
4. Commit the three changed files together — `tokens.json`, `tokens.md`, and the regenerated mirrors.
5. Run `groundx-slides` build (`cd templates && npm run build`) to verify the deck still looks right.
...
Mark it "legacy" or "deprecated" in the tables above with a note mapping to its replacement. Deprecated web aliases live in `groundx-web-ui/templates/constants.legacy.ts`; deprecated slide aliases live under the "Legacy color aliases" group in `tokens.json` (so they keep flowing through the generator into `:root`). Keep both until no call site references the old name, then remove from the legacy file and regenerate.
