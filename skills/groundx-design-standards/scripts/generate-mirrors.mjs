 ...
 *   - ../../groundx-web-ui/templates/constants.generated.ts
 *     (brand-token exports for React/MUI consumers)
 *   - Replaces the :root block in ../../groundx-slides/templates/styles.css
 ...
 * stay untouched.
 *
 * Run: node scripts/generate-mirrors.mjs (from the groundx-design-standards dir)
 */
...
const TS_OUT_PATH  = resolve(SKILLS_DIR, "groundx-web-ui/templates/constants.generated.ts");
const CSS_OUT_PATH = resolve(SKILLS_DIR, "groundx-slides/templates/styles.css");
...
function tokenPathToTsConst(pathParts) {
  ...
  // color.legacy.aqua → (skip — web-ui has its own TS legacy aliases in constants.legacy.ts)
  ...
  if (group === "color") {
    ...
    return null; // surfaceAlias and legacy are not exported to TS from this generator
  }
  if (group === "typography") {
...
function renderTs() {
  const lines = [];
  lines.push("/**");
  lines.push(" * AUTO-GENERATED by groundx-design-standards/scripts/generate-mirrors.mjs");
  lines.push(" * Source of truth: groundx-design-standards/tokens.json");
  lines.push(" *");
  lines.push(" * DO NOT EDIT THIS FILE BY HAND. Re-run the generator instead:");
  lines.push(" *   cd groundx-design-standards && node scripts/generate-mirrors.mjs");
  lines.push(" *");
  lines.push(" * Hand-written chrome tokens (drawerWidth, NAV_ICON_GREY, etc.) and legacy");
  lines.push(" * alias re-exports live in constants.chrome.ts and constants.legacy.ts.");
  lines.push(" * The barrel constants.ts re-exports all three.");
  lines.push(" */");
  lines.push("");
...
function renderCssRoot() {
  ...
  lines.push("/* Regenerate via: (cd groundx-design-standards && node scripts/generate-mirrors.mjs) */");
  lines.push("/* Source of truth: groundx-design-standards/tokens.json */");
  lines.push(":root {");
  ...
  const sections = [
    ...
    ["Legacy color aliases (§1.4)",             (p) => p[0] === "color" && p[1] === "legacy"],
