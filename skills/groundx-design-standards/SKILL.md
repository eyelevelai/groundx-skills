...
| `groundx-slides` | HTML → PDF (16:9 rendered via headless Chrome) | Palette → CSS `:root` vars in `styles.css`. Typography → `@import` THICCCBOI from the same CDN as the dashboard. Logos → static image placements. Principles → layout grid, chart styles. |
...
- **THICCCBOI is the font.** Weight ladder is 400 / 600 / 700 / 800. Body is 400; labels and CTAs are 600; headlines are 700; cover + display-stat numerals are 800. Weight 900 is retired.
...
- **Four logos, purposeful placement.** The wordmark (three treatments) plus the "A VALANTOR COMPANY" tagline. See `references/logos.md`.

## Brand relationship

GroundX is the product. EyeLevel is the AI company that builds it. Valantor is the parent company. The tagline under the GroundX wordmark reads "A VALANTOR COMPANY" — the EyeLevel name lives in company-level contexts (author bylines, legal footers, the `eyelevel.ai` domain), not in the logo lockup. The authoritative palette and typography are maintained in the Eyelevel webflow property; `references/styleguide.html` mirrors the live CSS.
...
assets/
├── logos/
│   ├── groundx-ai-log.png          Default (white + coral on navy; sidebars, hero panels)
│   ├── groundX-logo-color.png      Color on light backgrounds (marketing, docs, emails)
│   ├── groundx-logo-black.png      Black wordmark (print, grayscale, ink-only exports)
│   └── valantor-tagline.png        "A VALANTOR COMPANY" tagline — pair with the GroundX wordmark
└── fonts/
    └── README.md                   Where the THICCCBOI font files should go (and which formats)
```

Medium-specific skills reference these by relative path: `../groundx-design-standards/assets/logos/groundx-ai-log.png`.
...
2. **Preserve weight intent.** THICCCBOI's 600 is semibold-adjacent; if a medium can only use semibold (`600`) vs. bold (`700`), pick the one closer to the specified weight.
