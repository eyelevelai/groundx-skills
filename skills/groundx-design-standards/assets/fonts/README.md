...
This folder is reserved for local Inter font binaries in case a non-web medium (PowerPoint, Word, print) can't reach Google Fonts at render time, or a deliverable needs embedded fonts. Day-to-day, both the dashboard (`groundx-web-ui`) and the slide deck (`groundx-slides`) load Inter directly from Google Fonts — see `../../references/typography.md`.
...
- **`groundx-slides`**: `templates/styles.css` `@import`s Inter from Google Fonts. To run offline, swap the `@import` for local `@font-face` declarations pointing at files in this folder (or mirror into `groundx-slides/templates/fonts/`).
- **`groundx-web-ui`**: `templates/fonts.css` also uses a Google Fonts `@import`. Same offline swap applies.
